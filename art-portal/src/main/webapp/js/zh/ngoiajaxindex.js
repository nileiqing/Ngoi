...
var innewcid=36;//首页新闻对应的categoryid
...
//获取lab的介绍
   $.getJSON(allurl + "getlist.html?categoryId="+labinfocid, 
		function(data) {
        var content = data[0].contentZhCn;
...
//获取研究课题
	$.getJSON(allurl + "getlist.html?categoryId="+resthcid, 
        	...
        	$.each(data,function(i,item){
	            researchtho += "<li><span>"+item.contentZhCn+"</span></li>";
	        });
...
//获取首页新闻
	...
	$.getJSON(allurl + "getlistbyorder.html?categoryId="+innewcid+"&ordertype="+ordertype, 
	        	...
	        	$.each(data,function(i,item){
	        		...
	        		news+="<span class=span-date>["+item.subTitle+"]</span><b>&nbsp;</b><span class=span-desc><a href="+allurl+"zh/home/news/"+item.id+".html>"+item.title+"</a><b>&nbsp;</b>"+item.titleDesc+"<b>&nbsp;</b>...</span></div>";
...
//获取新闻详情
	...
	$.getJSON(allurl + "getcontent.html?contentId="+contentid, 
				...
				$(".main-news-desc").append("<div class=newsdate >发布于 "+data.subTitle+"</div>");
	});
});
