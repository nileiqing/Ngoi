...
//	detaillist.json
	if(parseInt(nodeid)==216){
		//$.getJSON("../json/teacher.json", function(res) {
		$.getJSON(allurl + "teacher.html", function(res) {
			$("#detailtitle").html("师资名录");
            $("#newsmodelnexttitle").html("师资名录");
            $("#newsmodelthirdtitle").html("");

            var detaillist = "<div id='detaillist'>";
            $.each(res.data,function(i,item){
            	detaillist += "<a class='detaillistone' style='cursor:default;' href='javascript:void(0)'><h1>" + item.sort + "</h1><br/>";
            	$.each(item.teacher,function(i,iteminner){
            		detaillist += "<span style='padding:5px 10px;cursor:pointer;color:black;' onclick='javascript:detail(" + iteminner.id + ")'>" + iteminner.name + "</span>";
            	});
            	detaillist += "</a>";
            });
            detaillist += "</div>";
