	...
	$.getJSON(allurl + "getalllist.html?categoryId="+publicationcid+"&ordertype="+ordertype,
				function(data) {
	        		...
	        		if(item.categoryId==booksid){
	        			...
	        			booklistr+="<li><a target=_self title='点击下载'  href="+item.pic2+">"+item.content+"</a></li>";

	        		}else if(item.categoryId==bookChaptersid){
						if(item.pic2==""){
							item.pic2 = "#";
						}
						bookChapterlistr+="<li><a  target=_self title='点击下载' href='#'>"+item.content+"</a></li>";
	        		}else if(item.categoryId==journalid){
						if(item.pic2==""){
							item.pic2 = "#";
						}
	        			journalistr+="<li><a target=_self title='点击下载' href="+item.pic2+">"+item.content+"</a></li>";
	        		}else if(item.categoryId==conferenceid){
						if(item.pic2==""){
							item.pic2 = "#";
						}
	        			conferencestr+="<li><a target=_self title='点击下载' href="+item.pic2+">"+item.content+"</a></li>";
	        		}else if(item.categoryId==patentsid){
						if(item.pic2==""){
							item.pic2 = "#";
						}
	        			patentstr+="<li><a target=_self title='点击下载'  href="+item.pic2+">"+item.content+"</a></li>";
	        		}
		        });
					if(booklistr!=""){
						
						finalHtml+="<div class=publication_list_title>出版书目:</div><b><hr class=mainlefthr></b><ol >"+booklistr+"</ol>";
					}
					if(bookChapterlistr!=""){
						finalHtml+="<div class=publication_list_title>出版章节:</div><b><hr class=mainlefthr></b><ol >"+bookChapterlistr+"</ol>";
					}
					if(journalistr!=""){
						finalHtml+="<div class=publication_list_title>论文:</div><b><hr class=mainlefthr></b><ol >"+journalistr+"</ol>";
					}if(conferencestr!=""){
						finalHtml+="<div class=publication_list_title>会议:</div><b><hr class=mainlefthr></b><ol >"+conferencestr+"</ol>";
					}
					if(patentstr!=""){
						finalHtml+="<div class=publication_list_title>专利:</div><b><hr class=mainlefthr></b><ol >"+patentstr+"</ol>";
					}
